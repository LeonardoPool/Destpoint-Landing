<script lang="ts">
	...
	import { getPromotions, getSupportImages, addPromotion } from '$lib/promoService';
	...
	onMount(() => {
		...
		const loadPexelsImages = async () => {
			try {
				...
				const explorePhotos = await searchPhotos('luxury travel vacation resort', 1);
				if (explorePhotos.length > 0) exploreImage = explorePhotos[0];

				const resortThumbPhotos = await searchPhotos('luxury hotel room', 1);
				if (resortThumbPhotos.length > 0) resortThumbImage = resortThumbPhotos[0];

				const guideAvatarPhotos = await searchPhotos('friendly tour guide portrait', 1);
				if (guideAvatarPhotos.length > 0) guideAvatarImage = guideAvatarPhotos[0];
			} catch (error) {
		...
		<div class="search-mockup-wrapper">
			...
			<div class="locked-overlay">
				<div class="lock-icon-container">
					<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="lock-svg">
...
<section class="promotions-section">
	...
	>
		...
		<button 
			...
			onclick={() => promotions.length > 0 && (currentPromoIndex = (currentPromoIndex - 1 + promotions.length) % promotions.length)}
			aria-label="Anterior"
		>
		...
		<div class="cards-stack">
			{#each promotions as promo, index}
				<div 
					...
					class:left={promotions.length > 1 && index === (currentPromoIndex - 1 + promotions.length) % promotions.length}
					class:right={promotions.length > 2 && index === (currentPromoIndex + 1) % promotions.length}
					onclick={() => {
		...
		<button 
			...
			onclick={() => promotions.length > 0 && (currentPromoIndex = (currentPromoIndex + 1) % promotions.length)}
...
<section class="why-us">
	<div class="why-us-container">
		...
		<div class="why-us-left">
			<h2 class="why-us-title">Descubre el mundo con experiencias diseñadas para ti</h2>
			<p class="why-us-desc">
				Creamos viajes que combinan destinos, hospedaje, transporte y actividades para que solo te preocupes por disfrutar cada momento.
			</p>
		...
		<div class="why-us-right">
			...
			<div class="image-grid">
				<div class="img-large">
					{#if beachImage}
						<img src={beachImage} alt="Riviera Maya" class="img-placeholder" style="object-fit: cover; width: 100%; height: 100%;" />
					{:else}
				...
				<div class="img-small-col">
					{#if eiffelImage}
						<img src={eiffelImage} alt="Cancún" class="img-placeholder" style="object-fit: cover; width: 100%; height: 100%;" />
					{:else}
						<div class="img-placeholder img-placeholder--eiffel"></div>
					{/if}
					{#if parisImage}
						<img src={parisImage} alt="Francia" class="img-placeholder" style="object-fit: cover; width: 100%; height: 100%;" />
					{:else}
	...
	<div class="explore-container">
		...
		<div class="explore-left">
			<div class="explore-image-wrapper">
				{#if exploreImage}
					<img src={exploreImage} alt="Explore destination" class="explore-image-placeholder" style="object-fit: cover; width: 100%; height: 100%;" />
				{:else}
				...
				<div class="resort-card">
					{#if resortThumbImage}
						<img src={resortThumbImage} alt={resort.name} class="resort-thumb" style="object-fit: cover;" />
					{:else}
				...
				<div class="guide-badge">
					{#if guideAvatarImage}
						<img src={guideAvatarImage} alt={resort.guide} class="guide-avatar" style="object-fit: cover;" />
					{:else}
		...
		<div class="explore-right">
			...
			<p class="explore-desc">
				Descubre destinos, consejos de viaje, experiencias auténticas y recomendaciones de nuestros expertos para planificar cada escapada con confianza.
			</p>
...
.search-mockup-wrapper {
	...
	border-radius: 20px;
	padding: 1px;
	background: linear-gradient(135deg, rgba(24, 74, 87, 0.1) 0%, rgba(24, 74, 87, 0.05) 100%);
